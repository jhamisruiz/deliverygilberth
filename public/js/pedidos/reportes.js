...
function getSum(start = null, end = null) {
	$.post(base_url + 'reportes/getSumDiario', {start:start, end:end}, function (data) {
    $("#total").html("");
    if(data!=null){
...
$('#btn_date_apply').click((e) => {
    ...
    getSum(year + "-" + month.padZero(2, '0') + "-" + day.padZero(2, '0'));
		tablaReportes.columns([1]).search(year + "-" + month.padZero(2, '0') + "-" + day.padZero(2, '0')).draw();

	}
	if (selection === 'range') {
		let date_start = $($('.actual_range')[0]).datepicker('getDate');
		date_start = date_start.getFullYear() + '-' + (date_start.getMonth() + 1).toString().padZero(2, '0') + '-' + (date_start.getDate()).toString().padZero(2, '0');
		let date_end = $($('.actual_range')[1]).datepicker('getDate');
		date_end = date_end.getFullYear() + '-' + (date_end.getMonth() + 1).toString().padZero(2, '0') + '-' + (date_end.getDate()).toString().padZero(2, '0');
